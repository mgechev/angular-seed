...
export class WebConfig {
   ...
   * @type {number}
   */
  WEB_DOCS_PORT = argv['docs-port'] || 4003;
   ...
   * @type {number}
   */
  WEB_COVERAGE_PORT = argv['coverage-port'] || 4004;
   ...
   * @type {string}
   */
  WEB_BOOTSTRAP_DIR = 'app';

  /**
   * The directory where the web files are located.
   * The default directory is `web`.
   * @type {string}
   */
  APP_WEB = argv['web'] || 'web';
   ...
   * @type {string}
   */
  WEB_BOOTSTRAP_MODULE = `${this.WEB_BOOTSTRAP_DIR}/` + (this.ENABLE_HOT_LOADING ? 'hot_loader_main' : 'main');
   ...
   * @type {string}
   */
  WEB_APP_TITLE = 'Welcome to angular2-seed!';

  /**
   * The base folder of the applications source files.
   * @type {string}
   */
  WEB_APP_SRC = `src/${this.APP_WEB}`;

  /**
   * The folder of the applications asset files.
   * @type {string}
   */
  WEB_ASSETS_SRC = `${this.WEB_APP_SRC}/assets`;

  /**
   * The folder of the applications css files.
   * @type {string}
   */
  WEB_CSS_SRC = `${this.WEB_APP_SRC}/css`;
   ...
   */
  TASKS_DIR = join(process.cwd(), this.TOOLS_DIR, 'tasks', 'seed');

  /**
   * The directory of the tasks provided by the seed.
   */
  WEB_TASKS_DIR = join(process.cwd(), this.TOOLS_DIR, 'tasks', 'frameworks', 'web', 'seed');
   ...
   * @type {string}
   */
  WEB_DEV_DEST = `${this.WEB_DIST_DIR}/dev`;

  /**
   * The folder for the built files in the `prod` environment.
   * @type {string}
   */
  WEB_PROD_DEST = `${this.WEB_DIST_DIR}/prod`;

  /**
   * The folder for temporary files.
   * @type {string}
   */
  WEB_TMP_DIR = `${this.WEB_DIST_DIR}/tmp`;

  /**
   * The folder for the built files, corresponding to the current environment.
   * @type {string}
   */
  WEB_APP_DEST = this.ENV === ENVIRONMENTS.DEVELOPMENT ? this.WEB_DEV_DEST : this.WEB_PROD_DEST;

  /**
   * The folder for the built CSS files.
   * @type {strings}
   */
  WEB_CSS_DEST = `${this.WEB_APP_DEST}/css`;

  /**
   * The folder for the built JavaScript files.
   * @type {string}
   */
  WEB_JS_DEST = `${this.WEB_APP_DEST}/js`;
   ...
   * @type {boolean}
   */
  WEB_ENABLE_SCSS = argv['scss'] || false;

  /**
   * The list of NPM dependcies to be injected in the `index.html`.
   * @type {WebInjectableDependency[]}
   */
  WEB_NPM_DEPENDENCIES: WebInjectableDependency[] = [
   ...
   * @type {WebInjectableDependency[]}
   */
  WEB_APP_ASSETS: WebInjectableDependency[] = [
    { src: `${this.WEB_CSS_SRC}/main.${ this.getInjectableStyleExtension() }`, inject: true, vendor: false },
  ];
   ...
   * @return {WebInjectableDependency[]} The array of npm dependencies and assets.
   */
  get DEPENDENCIES(): WebInjectableDependency[] {
    return normalizeDependencies(this.WEB_NPM_DEPENDENCIES.filter(filterDependency.bind(null, this.ENV)))
      .concat(this.WEB_APP_ASSETS.filter(filterDependency.bind(null, this.ENV)));
  }
  ...
  protected SYSTEM_CONFIG_DEV: any = {
    ...
    packageConfigPaths: [
      `${this.WEB_APP_BASE}node_modules/*/package.json`,
      `${this.WEB_APP_BASE}node_modules/**/package.json`,
      `${this.WEB_APP_BASE}node_modules/@angular/*/package.json`
    ],
    paths: {
      [this.WEB_BOOTSTRAP_MODULE]: `${this.WEB_APP_BASE}${this.WEB_BOOTSTRAP_MODULE}`,
      '@angular/core': `${this.WEB_APP_BASE}node_modules/@angular/core/core.umd.js`,
      '@angular/common': `${this.WEB_APP_BASE}node_modules/@angular/common/common.umd.js`,
      '@angular/compiler': `${this.WEB_APP_BASE}node_modules/@angular/compiler/compiler.umd.js`,
      '@angular/http': `${this.WEB_APP_BASE}node_modules/@angular/http/http.umd.js`,
      '@angular/router': `${this.WEB_APP_BASE}node_modules/@angular/router/router.umd.js`,
      '@angular/platform-browser': `${this.WEB_APP_BASE}node_modules/@angular/platform-browser/platform-browser.umd.js`,
      '@angular/platform-browser-dynamic':
        `${this.WEB_APP_BASE}node_modules/@angular/platform-browser-dynamic/platform-browser-dynamic.umd.js`,
      'rxjs/*': `${this.WEB_APP_BASE}node_modules/rxjs/*`,
      'app/*': `/app/*`,
      '*': `${this.WEB_APP_BASE}node_modules/*`
    },
    packages: {
  ...
  SYSTEM_BUILDER_CONFIG: any = {
    defaultJSExtensions: true,
    packageConfigPaths: [
      join(this.WEB_PROJECT_ROOT, 'node_modules', '*', 'package.json'),
      join(this.WEB_PROJECT_ROOT, 'node_modules', '@angular', '*', 'package.json')
    ],
    paths: {
      [`${this.WEB_TMP_DIR}/*`]: `${this.WEB_TMP_DIR}/*`,
      '*': 'node_modules/*'
    },
    packages: {
      '@angular/core': {
  ...
  PLUGIN_CONFIGS: any = {
     ...
     */
    'browser-sync': {
      middleware: [require('connect-history-api-fallback')({ index: `${this.WEB_APP_BASE}index.html` })],
      port: this.WEB_PORT,
      startPath: this.WEB_APP_BASE,
      open: argv['b'] ? false : true,
      injectChanges: false,
      server: {
        baseDir: `${this.WEB_DIST_DIR}/empty/`,
        routes: {
          [`${this.WEB_APP_BASE}${this.WEB_APP_DEST}`]: this.WEB_APP_DEST,
          [`${this.WEB_APP_BASE}node_modules`]: 'node_modules',
          [`${this.WEB_APP_BASE.replace(/\/$/, '')}`]: this.WEB_APP_DEST
        }
      }
    }
  };
  ...
  getInjectableStyleExtension() {
    return this.ENV === ENVIRONMENTS.PRODUCTION && this.WEB_ENABLE_SCSS ? 'scss' : 'css';
  }
 ...
 * @param {WebInjectableDependency[]} deps - The dependencies to be normalized.
 */
export function normalizeDependencies(deps: WebInjectableDependency[]) {
  deps
    .filter((d: WebInjectableDependency) => !/\*/.test(d.src)) // Skip globs
    .forEach((d: WebInjectableDependency) => d.src = require.resolve(d.src));
  return deps;
}
 ...
 * @param  {WebInjectableDependency} d   - The dependency to check.
 * @return {boolean}                    `true` if the dependency is used in this environment, `false` otherwise.
 */
function filterDependency(env: string, d: WebInjectableDependency): boolean {
  if (!d.env) {
...
function appVersion(): number | string {
  var pkg = require('../../../../src/web/package.json');
  return pkg.version;
}
...
function customRules(): string[] {
  var lintConf = require('../../../../src/web/tslint.json');
