...
gulp.task('build.js.dev', function () {
    var result = gulp.src(PATH.src.all+'/**/*ts')
        .pipe(plumber())
...
gulp.task('build.assets.dev', ['build.js.dev'], function () {
    return gulp.src([PATH.src.all+'/**/*.html', PATH.src.all+'/**/*.css'])
        .pipe(gulp.dest(PATH.dest.dev.all));
});

gulp.task('build.index.dev', function() {
    var target = gulp.src(injectableDevAssetsRef(), { read: false });
    return gulp.src(PATH.src.all+'/index.html')
...
gulp.task('build.js.tmp', function () {
    var result = gulp.src([PATH.src.all+'/**/*ts', '!'+PATH.src.all+'/init.ts'])
        .pipe(plumber())
        .pipe(tsc(tsProject));
...
gulp.task('build.init.prod', function() {
    var result = gulp.src(PATH.src.all+'/init.ts')
        .pipe(plumber())
        .pipe(sourcemaps.init())
        .pipe(tsc(tsProject));
